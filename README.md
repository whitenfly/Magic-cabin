...
**当前阶段：`J4` 系统模块化 🟡 开发中**（**`J4` 已全部完成（含债务 C1–C7 清零）**：`legacy/` 删除（6217 行 → 21 段 + 21 行骨架）、`animate()` 4 行、`aim` 通路合一、镜面涟漪接回、光源归属修正、室外片切分。逐任务结果见 [`docs/实施结果/`](./docs/实施结果/) 下的 `J4.*-实施结果.md`）
**当前稳定版：`v0.3.0`**（tag `v0.3.0`，开发主线在 `dev` 分支；本阶段的任务快照 tag 见（最新 `v0.4.0-dev.10`） [`docs/VERSIONING.md`](./docs/VERSIONING.md) §5.4）
