...
**当前阶段：`J2.5` 配置编排层 ✅ 已完成**（插层子阶段，版本 `0.2.5`，见 [`docs/实施结果/J2.5-配置编排层-实施结果.md`](./docs/实施结果/J2.5-配置编排层-实施结果.md)）
...
`J2.5` 再把它接到配置层 —— **改一个字段，设置面板自己长出控件、值自己存进 `localStorage`、场景自己跟着变**；
**关一个布尔值，那个模块的代码根本不会被下载**。所以 `J3` 的每件物件从"要不要补一个注册"变成**填空**。
画面与性能**与阶段开始前完全相同**：

| 判据 | 结果 |
|---|---|
| 像素回归 | 3 机位 sha256 **逐字节相同**（`J2.5` 3/3、`J2` 全程 10/10 次） |
| 交互冒烟 | **28 / 0**（`J2.5` 与 `J2` 各轮均全绿） |
| 性能 | 3138 calls / 86158 tris —— **+0.0%**，一个指标都没动 |
| 单元测试 | **151 项**（零新依赖，Node 内置 `node:test`） |
