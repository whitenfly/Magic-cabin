...
**当前阶段：`J4` 系统模块化 🟡 开发中**（玩法系统逐个搬出 `legacy/`：**段切片基础设施已就位**，此后每个任务是"剪一段 + 六项门禁"；逐任务的进度见 [`docs/实施结果/`](./docs/实施结果/) 下的 `J4.*-实施结果.md`）
**当前稳定版：`v0.3.0`**（tag `v0.3.0`，开发主线在 `dev` 分支；本阶段的任务快照 tag 见（最新 `v0.4.0-dev.5`） [`docs/VERSIONING.md`](./docs/VERSIONING.md) §5.4）
