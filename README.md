...
**当前阶段：`J5` 内容管线与静态页 ⬜ 计划**（`J4` 系统模块化已**发布为 `v0.4.0`**：`legacy/` 删除（6217 行 → 21 段 + 21 行骨架）、`animate()` 4 行、**每帧分支归位**、`aim` 通路合一、镜面涟漪接回、光源归属修正、室外片切分。逐任务结果见 [`docs/实施结果/`](./docs/实施结果/) 下的 `J4.*-实施结果.md`）
**当前稳定版：`v0.4.0`**（tag `v0.4.0`，2026-09-17；开发主线在 `dev` 分支；`J4` 的任务快照 tag 见（最新 `v0.4.0-dev.16`） [`docs/VERSIONING.md`](./docs/VERSIONING.md) §5.4）
上一步：**`J3` 物件模块化 ✅ 已完成**（基础设施 100% + 35/67 件搬迁，其余 32 件因四类结构性阻塞交由 `J4`；含 `J3.1` 交互通路回归修复 —— 见 [`docs/实施结果/J3-实施结果.md`](./docs/实施结果/J3-实施结果.md)、[`J3.1-实施结果.md`](./docs/实施结果/J3.1-实施结果.md)）
`J4` 的目标：player / weather / audio / magic / ui 逐个搬进 `systems/**`，最后删除 `legacy/` ——
