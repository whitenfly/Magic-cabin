...
export const cabinConfig = {
  ...
  time: {
    ...
    hour: 10,
    // 时间流速，单位是**游戏秒 / 真实秒**：0 = 冻结，60 = 现实 1 秒等于游戏 1 分钟
    // ⚠️ 这里只是"场景初始条件"的读数出口；用户在面板里调的那个值住在
    //    `src/config/settings.config.js` 的 `'time.scale'`（它把本字段当默认值，两边必须一致）。
    //    菜单里的流速滑杆是**非线性**的（前半个行程 0–60×、后半个 60×–3600×），
    //    曲线定义见 `settings.config.js` 的 `TIME_SCALE_CURVE`。
    scale: 60,
  },
