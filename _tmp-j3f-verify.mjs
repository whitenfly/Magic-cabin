/**
 * J3-F 逐字节核对器（**应用前**用的，临时工具，非交付物）
 *
 * ⚠️ 本脚本按**行号**读 monolith（B4 那 6 件应用前的行号：几何段 L712–1525、每帧块 L6684–7228）。
 * B4 的 6 件已于提交 `87843b1` 应用进 monolith，行号整体前移（7521 → 7065 行）
 * ⇒ **本脚本现在必然报残差**，那是"已经搬走了"的正常结果，不是回归。
 * **应用后的核对请跑 `_tmp-j3f-verify-applied.mjs`。**
 *
 * 它的历史价值：在写盘**之前**证明"搬进模块的每一行都与 monolith 原行逐字相同"——
 * 当时 6 件的残差全为 0：
 * ```
 *   floor1/orrery         几何 32/32  每帧 10/10
 *   floor1/potion-bottle  几何 78/78  每帧 30/30
 *   floor1/dining-book    几何 34/34  每帧 35/35
 *   floor1/cauldron       几何 88/88  每帧 38/38（其中 3 行是外层注释头 + 花括号）
 *   floor1/long-table     几何 42/42  每帧  4/4
 *   floor1/tableware      几何 69/69  每帧  9/9
 * ```
 *
 * 对每件已搬迁的物件做三件事：
