# 版本管理执行规范 · SPEC-1.0.2 ★ 强制执行

<!-- ⛔ 修改本文件前必须先读 §12。规范是执行判据，禁止"顺手改"。 -->

| 项 | 值 |
|---|---|
| 规范版本 | **SPEC-1.0.2**（修订历史见 §12.4） |
...
> ⇒ **已从账本移除**。历史遗留的 `task/J3` 按"历史不动"原则保留，见 **§2.5**；
> §2.3 的查重 ① 仍须检查该编号是否被它占用（防止 R6 的编号复用）。
...
| `task/J3` tag | **`task/<编号>` tag 这一形态的孤例**（`J2.1`–`J2.10`、`J2.5`、`J3.1` 及之后**都没打**）；与 `v0.3.0-dev.1` 指向**同一提交**，是纯同义标记、不携带额外信息 | **形态已废止**（SPEC-1.0.2，见 §2.1 注）：现行流程**不再产生**新的。此 tag **保留不删**（§3.6「tag 打上就不删」）、**不推**（`ship` 只收 `v*`）；仅在 §2.3 查重 ① 里作为"该编号已被占用"的依据 |
...
| ~~tag `task/<编号>`~~ | —— **形态已废止**（§2.1 注 / §2.5）：现行流程两条路径（工具 / 手工等价）**都不产生**它 | 历史遗留的**不删** | 不推（`ship` 只收 `v*`） |
...
- 本文件当前版本 **SPEC-1.0.2**，处于**执行期冻结**状态。
- 「轻易不可修改」的**具体含义**：
