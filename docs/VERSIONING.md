...
| 5 | **分支 tag**（可选） | `task/<编号>` | `task/J4.1` | 全局 | 手工（工具不打） |
| 6 | **开发版 tag** | `v0.<x>.0-dev.<N>` | `v0.4.0-dev.3` | base 内 | `task:done` |
| 7 | **正式版 tag** | `v0.<x>.0` ／ `v0.<x>.5` | `v0.4.0` | 全局 | 发布（§5） |
| 8 | **实施结果文档** | `docs/实施结果/<编号>-实施结果.md` | `J4.1-实施结果.md` | 全局 | 开发者 |
| 9 | **提交信息前缀** | `<编号> <摘要>` | `J4.1 player 模块化：…` | — | 开发者 |

**读数口诀**：编号 `J4.1` 一出现，它的分支（`task/J4.1-…`）、tag（`task/J4.1`）、文档（`J4.1-实施结果.md`）、提交（`J4.1 …`）**全部同号**，四处同名即为合法。
...
```bash
NUM=J4.1                                    # ← 换成要用的编号
git tag -l "task/$NUM"                      # ① 分支 tag 是否已存在
git log --oneline --grep="$NUM"             # ② 历史提交是否用过这个编号
...
| `task/J3` tag | 手工打的"人读便利"tag，与 `v0.3.0-dev.1` 同提交 | 工具**不打**此类 tag；`ship` 也**不推**（只收 `v*` 开头） |
...
| **tag** `task/<编号>` | 手工（工具不打） | 不删 | 不推（`ship` 只收 `v*`） |
...
#   ← 唯一允许在 main 上出现的开发类动作，到此为止

# ★ R3 的豁免：发布元数据提交（只改 package.json 的 version 一个字段）
#   手改 package.json：version → 0.4.0
git commit -am "chore(release): v0.4.0"
git tag -a v0.4.0 -m "J4 系统模块化：legacy/ 清零 + animate 收缩为调度骨架"
...
> **豁免边界（严格）**：`main` 上只允许 ① merge commit ② 紧跟其后、`push` 之前的那**一次** `chore(release)` 元数据提交。
> 任何其他提交（功能、修复、文档、重构）出现在 `main` 上 ⇒ 违反 R1。

### 5.3 ★ 阶段收尾清单（四处状态副本，**必须一起改**）

阶段状态在四个地方各有一份，历史上已多次漏同步（判例 P4）。收尾时**逐条打勾**：
...
> 判定：四处状态一致；不一致时以**本文件 §5.4** 为准（它是版本事实的唯一来源）。
...
| `0.3.0` | `v0.3.0` | `J3` 物件模块化 + `J3.1` 交互通路回归修复 | —— | 🟡 **开发中**（快照 `v0.3.0-dev.1`、`dev.2`；**待发布**） |
| `0.4.0` | `v0.4.0` | `J4` 系统模块化（`legacy/` 清零） | —— | ⬜ 计划 |
...
| **P4** | 阶段状态在**四处副本**（§5.4 表 / README / 路线图总表 / 路线图章节标题）各有一份，历史上多次漏同步 | **§5.3 阶段收尾清单**；状态只允许三种取值 |
