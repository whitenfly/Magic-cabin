...
| `0.2.5` | `v0.2.5` | `J2.5` 配置编排层（`settings.config.js` + `display.config.js` + `SettingsForm` + `blog/registry.js` + `verify:cf`） | 2026-09-15 | ✅ **稳定版**（任务快照 `v0.2.5-dev.1`） |
...
>
> **`.5` 插层子阶段怎么用**（`J2.5` 的实测先例）：子阶段自成一条版本线，
> 用 `--base=0.<x>.5` —— `J2.5` 因此拿到 `v0.2.5-dev.1` 这一级的任务快照，
> 而**不必**挤进 `J2` 那 11 个任务共用的 `0.2.0-dev.N` 序列（`J1.5` 是同类先例，
> 它直接以 `v0.1.5` 发布）。判据很简单：**看 `01-完善路线图.md` §2 的「版本号」列**，
> 那一列写的就是这条线该用的基数。
