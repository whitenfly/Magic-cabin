# 版本管理执行规范 · SPEC-1.3.1 ★ 强制执行

<!-- ⛔ 修改本文件前必须先读 §12。规范是执行判据，禁止"顺手改"。 -->

| 项 | 值 |
|---|---|
| 规范版本 | **SPEC-1.3.1**（修订历史见 §12.4） |
...
| # | 禁令 | 判定命令（必须为空输出） |
|---|---|---|
| **禁 1** | **禁止在 `dev` 或 `main` 上做开发提交** —— 已由**两层机械防护**强制（见 §9.3） | ★ **本判据的唯一定义处**（§0.3② / §1 R1 / §4⑥ / 附录 A A8 / QA Q3 均**引用此处**，不重复描述 —— §12.5 第 2 条）：<br>`git log --oneline --no-merges --first-parent "$(git describe --tags --abbrev=0 dev)..dev"`（应为空；`--first-parent` 不可省，理由见下方注） |
| **禁 2** | **禁止在 `dev` 上打正式版 tag**（不带 `-dev.` 的） | 见 §1 R2 |
| **禁 3** | **禁止在任务分支之外执行 `task:done`** | 工具已强制（在 `dev`/`main` 上直接 `die`） |

> #### ★ 禁 1 / R1 的判定命令 —— **唯一定义处**（SPEC-1.3.1）
>
> ```bash
> git log --oneline --no-merges --first-parent "$(git describe --tags --abbrev=0 dev)..dev"   # 应为空
> ```
>
> **`--first-parent` 不可省** —— 判据要抓的是「**没有经过任何合并就落在 `dev` 上**的提交」：
>
> | 写法 | 后果 |
> |---|---|
> | 只有 `--no-merges`（SPEC-1.3.0 及以前） | ★ **假红**：`--no-merges` 只滤掉「merge 这个提交本身」，**不滤掉「经由 merge 到达的提交」**。任务分支提交、以及 §4⑥.1 的**回填提交**都住在合并的**第二父侧**，照样被列出来；而回填被规则**规定不打新 tag** ⇒ **永久**停留在最新 tag 之后 ⇒ 判据必然非空 |
> | `--no-merges --first-parent`（现行） | ✓ 遍历只走 `dev` 自己那一侧、跳过全部第二父侧 ⇒ 回填提交与任务提交**天然不在其中**，真直提**必然在其中** |
>
> **与规则本意一致**：§3.2 的「唯一判据」是"任何改动都必须能追溯到某次 `--no-ff` 合并" ——
> 第一父链上没有非 merge 提交 ⟺ 所有非 merge 提交都在第二父侧 ⟺ 全部经合并引入。
> 它还**顺带守住 R4**：若用不带 `--no-ff` 的合并（ff），任务提交会落到第一父链上 ⇒ 同样被抓。
>
> ⚠️ **禁止**改用「按提交信息排除回填提交」之类的过滤 —— 那会让判据**可被伪造**
> （给真直提起个 `docs(<编号>): 回填` 的名字即可绕过）。判据必须建立在**分支拓扑**上。
>
> 依据与 A/B 实证见 §12.4 的 **SPEC-1.3.1** 行；为什么这么改见 [`VERSIONING-QA.md`](./VERSIONING-QA.md) **Q22**。
...
② **禁 1 / R1**（`dev` 上无直提）：命令**唯一定义在 §0.2**，此处不重复描述（§12.5 第 2 条）——
收尾时**必须**到 §0.2 取用该命令，并确认其输出为空。
...
| # | 红线 | 判定方式 |
|---|---|---|
| **R1** | **禁止在 `dev` / `main` 上做开发提交**（`feat` / `fix` / `chore` / `refactor` / `docs` / `test`）。所有改动**必须**在 `task/*` 分支上产生，并以 `--no-ff` 合并进入。★ **已由两层机械防护强制**（§9.3）：模型路径 `exec` 入口直接拒绝，人工路径 `pre-commit` 钩子拒绝 | ★ 判定命令**唯一定义在 §0.2**（此处不重复描述，§12.5 第 2 条）—— 该命令输出为空；且违规尝试会留下 `kind: blocked` 记录 |
...
- **自检**（**必须**跑）：
  ...
  ```
  - 禁 1 / R1 的判定命令**唯一定义在 §0.2** —— 收尾时到那里取用（此处不重复描述，§12.5 第 2 条）。

  > ⚠️ **走过回填路径（⑥.1）的收尾尤其要跑这一条**：回填提交是**非 merge 提交**且**不打新 tag**，
  > 用 SPEC-1.3.0 及以前的判据（缺 `--first-parent`）会把它**误报成 `dev` 直提** —— 见 §0.2 的注。
...
| `0.4.0` | `v0.4.0` | `J4` 系统模块化（`legacy/` 清零 + `animate()` 收缩为调度骨架 + **每帧分支归位**）+ 版本规范 `SPEC-1.3.0`→`1.3.1` | 2026-09-17 | ✅ **稳定版**（任务快照 `v0.4.0-dev.1`–`dev.16`） |
...
- 本文件当前版本 **SPEC-1.3.1**，处于**执行期冻结**状态。
- 「轻易不可修改」的**具体含义**：
...
| **1.3.1** | 2026-09-16 | **修错，不改变任何规则含义**（补丁位）：**① 禁 1 / R1 的判定命令由 `--no-merges` 修正为 `--no-merges --first-parent`** —— 原命令把「**没有经过任何合并就落在 `dev` 上**」偷换成了「最新 tag 之后存在非 merge 提交」，而 `--no-merges` 只滤掉「merge 这个提交本身」、**不滤掉「经由 merge 到达的提交」**，于是 §4⑥.1 的回填提交（规则**规定不打新 tag** ⇒ **永久**停留在最新 tag 之后）被**假红**（`J4.7` §2.6 实测 3 个；该任务**按 §12.5 第 4 条未擅自改规范**，只留事实待修）。新命令按**分支拓扑**判定 —— 第一父链上无非 merge 提交 ⟺ 所有非 merge 提交都在第二父侧 ⟺ 全部经 `--no-ff` 合并引入，与 §3.2「唯一判据」一致，并**顺带守住 R4**（ff 合并会落到第一父链上 ⇒ 被抓）。**② 同一判据由 6 处重复描述收敛为 §0.2 单一定义 + 5 处引用**（§0.3② / §1 R1 / §4⑥ / 附录 A A8 改为引用；QA Q3 同步），依据 §12.5 第 2 条。**③ 附录 A A10 由「四处 / 4/4」更正为「五处 / 5/5」** —— §5.3 已于 SPEC-1.1.0 补入第 5 处，A10 漏改。**④ `J4.7` §2.6 备选的"存在后代 merge"判据经 A/B 实测为恒真**（真直提之后的任何 merge 都是它的后代 ⇒ 对 `J3` 的 6 个真直提**全部判为通过**）⇒ **不予采用**，`scripts/oneoff/_j4-ancestry-check.mjs` 一并退役。A/B 实证：`--first-parent` 在 `J3` 区间（`v0.2.5-dev.1..v0.3.0-dev.1`）抓到 **6/6** 真违规、在 `J4` 区间（`v0.4.0-dev.1..dev`）对回填与任务分支提交 **0 误报**。同步：`VERSIONING-QA.md` Q3 + 新增 **Q22** | `J4.13` |
...
| A8 | 收尾后无 dev 直提 | ★ 见 **§0.2** 禁 1 的唯一定义处（此处不重复描述） | 空 |
| A9 | 推送已完成 | `node scripts/release.mjs status` | dev/main 均 0 ahead、tag 全 ✓ |
| A10 | 阶段收尾时五处状态一致 | §5.3 清单 | 5/5 已改 |
