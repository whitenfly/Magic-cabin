...
| **J3** | 🟡 **基础设施已完成 + 21/67 件搬迁**：`defineProp` 契约（`app/defineProp.js`）、装配器（`app/installProp.js`）、**`mounts.js` 挂载点 ID 表**、惰性装配环境、搬迁应用器（`scripts/oneoff/_j3-apply.mjs`）与两条新门禁（`tests/e2e/j3-probe.mjs`、场景图比对脚本）。其余 46 件因**四类结构性阻塞**（每帧分支与主循环耦合 / 光源槽位 / `addStatic` 链 / 自建射线）留给 `J4` —— 见 [`实施结果/J3-实施结果.md`](./实施结果/J3-实施结果.md) §3 | `BuildPlaning/01` §3 |
