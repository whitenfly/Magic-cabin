...
console.log('\n【④ 关键标识符计数（搬迁后 vs 源文件）】')
{
  ...
  const KNOWN_DELTA = {
    ...
    //
    // ★ J4.43 `floor2/board`（18.8 整段，336 行）⇒ −32 更新为 **−35**：
    //   这一段一口气删掉 **5** 处 regMagic 调用（**3 张便签各一条 + 板擦一条 + 粉笔一条**）
    //   ⇒ 真实调用 **−5**；`board.js` 用 `interactables()` 声明 **5 条**（同样 3 便签 + 板擦 + 粉笔），
    //   **不含 regMagic 调用** ⇒ 不回补。
    //   ⚠️ 但实测是 **−3**（`64 → 29`）而不是 −5 —— 因为 18.8 段的**行注释里原有 2 处**
    //   「函数名 + 左括号」的字面量（搬迁对照表），它们随整段一起被删除
