// F1 阶段的源码组装脚本：
//   ① 抽取 UI DOM 为共享模块 src/cabin/dom.js（唯一真值来源）
//   ② 生成 index.html（同一入口同时支持 Vite 构建与零构建静态服务）
// 后续 DOM 变更请直接改 src/cabin/dom.js 后重跑本脚本。
import fs from 'node:fs'
import path from 'node:path'

const SRC = 'D:/FireflyQAQ/Project/FrontProj/line-art-style-magic-cabin-main/index.html'
const ROOT = 'D:/FireflyQAQ/Project/FrontProj/Magic-cabin'

const lines = fs.readFileSync(SRC, 'utf8').split(/\r?\n/)
// 原 body 中的 UI DOM：第 753–835 行（<body> 在 752，三个 <script> 从 837 起）
const uiDom = lines.slice(752, 835).join('\n').replace(/\s+$/, '')

// ── ① src/cabin/dom.js ──
const domJs = `/**
 * 魔法小屋 —— UI DOM（原 index.html 第 753–835 行原样搬迁）
 *
 * 这些节点的 id / class 被实现代码大量依赖（\`getElementById\`），
 * **不得改名、不得改变结构**，否则交互会静默失效。
 *
 * 之所以做成模块常量而不是写死在 HTML 里：
 *   UI 节点必须在 legacy 实现执行**之前**存在于 DOM 中。
 *   由 JS 注入可以保证这个顺序（HTML 里写死则依赖解析顺序，易碎）。
 *
 * F3 阶段起，这些节点会逐步改由 src/cabin/systems/ui/ 动态创建，
 * 届时本文件会被删除。
 */
export const UI_HTML = \`
${uiDom}
\`

/** 把 UI DOM 插入 body（必须在 legacy 实现执行前调用） */
export function mountUI(root = document.body) {
  const holder = document.createElement('div')
  holder.innerHTML = UI_HTML
  const frag = document.createDocumentFragment()
  while (holder.firstChild) frag.appendChild(holder.firstChild)
  root.insertBefore(frag, root.firstChild)
}
`
fs.writeFileSync(path.join(ROOT, 'src/cabin/dom.js'), domJs, 'utf8')

// ── ② index.html ──
const html = `<!DOCTYPE html>
<html lang="zh">

<head>
    <meta charset="utf-8">
    <meta name="viewport"
        content="width=device-width, initial-scale=1, maximum-scale=1.0, user-scalable=no, viewport-fit=cover">
    <title>线稿风格魔女小屋</title>
    <meta name="description" content="一个线稿风格的 3D 魔法小屋。操控一只软软的史莱姆在魔法小屋里生活。">

    <link rel="stylesheet" href="./src/styles/cabin.css">

    <!--
      魔法小屋 —— 重构骨架（F0/F1 阶段）
      ====================================================================
      搬迁来源：line-art-style-magic-cabin-main/index.html（单文件 9811 行）
        · 样式   → src/styles/cabin.css             （原 10–748 行）
        · UI DOM → src/cabin/dom.js                 （原 753–835 行）
        · 实现   → src/cabin/legacy/monolith.js     （原 844–9807 行）

      本阶段原则：**只搬位置，不改实现**。页面表现与重构前完全一致。

      两种运行方式（同一个入口，无需改动）：
        A. Vite：      pnpm dev / pnpm build        —— 主力路径，正常环境下可用
        B. 零构建：    pnpm serve                   —— 兜底路径，浏览器直接解析 ESM
           （B 依赖下面的 importmap 解析裸模块名 \`three\`；
             A 模式下 Vite 自行解析依赖，此表不参与）

      门厅策略（ArtLine-Part/03 §8.3、08 §4.4）：
      F6 阶段将改为「门厅 HTML 先渲染 → 3D 空闲时挂载」，届时本文件会新增
      文章列表与 <noscript> 降级内容。
    -->
    <script type="importmap">
        {
            "imports": {
                "three": "./node_modules/three/build/three.module.js"
            }
        }
    </script>
</head>

<body>
    <script type="module" src="./src/main.js"></script>
</body>

</html>
`
fs.writeFileSync(path.join(ROOT, 'index.html'), html, 'utf8')

console.log('src/cabin/dom.js →', domJs.split('\n').length, '行')
console.log('index.html       →', html.split('\n').length, '行')
