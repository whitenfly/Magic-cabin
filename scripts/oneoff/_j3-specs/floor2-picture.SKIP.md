# 跳过（**已作废**）：`floor2/picture`（18.13 前墙挂画，镜子旁，点击编辑链接，支持 gif 动图） —— `J3` / B4

> ## ⚠️ 本文件已被 [`floor2-picture.json`](./floor2-picture.json)（`B5`）**取代** —— 保留仅作决策留痕。
>
> B4 的判断是「应用器只做整段替换、摘不掉区间外那 4 行」⇒ 一旦删了几何段就 `ReferenceError`。
> **B5 时应用器已经支持 `spec.tick`**（逐字替换任意一处唯一原文）——正是 `floor2/mirror`（8 行）
> 与 `floor2/junkBoxes`（6 行）用过的同一机制。于是 §4 那份"现成配方"**已按原样执行**：
> `startMarker` / `endMarker` / `layout` / `interactables` / `update` / 人工步骤② 全部落地，
> **GIF 动图这个功能完整保住**，只是"人工步骤"由 spec 声明、应用器代劳。
> 本文件 §1–§3 保留作 B4 决策过程的记录；§4 是已执行的配方；§5 的对比结论仍然成立。

> 本目录 `*.json` 才会被 `_j3-apply.mjs` 读；这是一份**说明**（`.md`），故意不写 spec，
> 否则别人跑一次不带 `--only` 的应用器就会把下面这个 `ReferenceError` 写进 `monolith.js`。
> 结论：**B4 轮不搬** —— 依据任务书硬约束 4「**区间外被引用的变量**（如挂画的 gif/链接状态被
